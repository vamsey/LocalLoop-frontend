        ...
        <div className={`flex-1 min-w-0 ${listMode ? "" : "flex flex-col gap-3"}`}>
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <h3 style={{ fontFamily: "'Outfit', sans-serif" }} className={`font-bold text-xl leading-tight truncate
...
function Explore() {
  ...
  return (
    ...
    >
      ...
      <div className={`sticky top-[73px] z-40 backdrop-blur-2xl border-b px-6 py-4 transition-colors duration-500
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className={`flex items-center gap-1 rounded-xl p-1.5 border
            ...
            <button onClick={() => setViewMode("grid")} className={`p-2 rounded-lg transition-all duration-200 ${viewMode === "grid" ? "bg-zinc-900 dark:bg-white text-white dark:text-black shadow" : darkMode ? "text-zinc-500 hover:text-white" : "text-zinc-500 hover:text-zinc-900"}`}>
              <Grid3X3 size={16} />
            </button>
            <button onClick={() => setViewMode("list")} className={`p-2 rounded-lg transition-all duration-200 ${viewMode === "list" ? "bg-zinc-900 dark:bg-white text-white dark:text-black shadow" : darkMode ? "text-zinc-500 hover:text-white" : "text-zinc-500 hover:text-zinc-900"}`}>
              <List size={16} />
            </button>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          /* ── EMPTY STATE ─────────────────────────────────────────────── */
            ...
            >
              ...
              <h2 style={{ fontFamily: "'Outfit', sans-serif" }} className={`text-3xl font-bold tracking-tight ${darkMode ? "text-white" : "text-zinc-900"}`}>
                No businesses found
              </h2>
      ...
      {/* ── MOCK MENU / DETAILS MODAL OVERLAY ── */}
      <AnimatePresence>
        {selectedBusiness && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            >
              <div className={`shrink-0 flex items-start justify-between p-8 border-b ${
                ...
                <div className="flex items-start gap-5">
                  ...
                  <div>
                    <h2 style={{ fontFamily: "'Outfit', sans-serif" }} className="text-3xl font-bold leading-tight tracking-tight">{selectedBusiness.name}</h2>
              ...
              <div className="flex-1 overflow-y-auto p-8 hide-scrollbar">
                <p className={`text-base leading-relaxed mb-8 pb-8 border-b ${darkMode ? "text-zinc-400 border-white/[0.08]" : "text-zinc-600 border-zinc-100"}`}>
                  {selectedBusiness.description}
                </p>

                <div className="flex items-center gap-2 mb-6">
                  <ShoppingBag className="text-violet-500" size={20} />
                  <h3 style={{ fontFamily: "'Outfit', sans-serif" }} className="text-xl font-bold">Catalog & Services</h3>
                </div>

                <div className="space-y-4">
                  {getMockMenu(selectedBusiness.category).map((item, idx) => (
                    <div key={idx} className={`p-5 rounded-2xl border flex justify-between items-center transition-all hover:scale-[1.02] ${
                      darkMode ? "bg-white/[0.02] border-white/[0.08] hover:border-violet-500/50" : "bg-white border-zinc-200 shadow-sm hover:border-violet-300"
                    }`}>
                      <div>
              ...
              <div className={`shrink-0 p-8 border-t flex flex-col sm:flex-row gap-4 ${
                ...
                <button className={`flex-1 flex items-center justify-center gap-2 py-4 rounded-2xl font-semibold border transition-colors ${
                  darkMode ? "border-white/[0.08] hover:bg-white/[0.05] text-white" : "border-zinc-300 hover:bg-zinc-100 text-zinc-900 bg-white"
                }`}>
                  <Phone size={18} />
